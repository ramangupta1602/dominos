...
const PizzaComponent = ({ pizza }) => {
  ...
  return (
    <View>
      ...
      {data.map((item, index) => (
        <Pressable key={index} style={{ borderEndColor: "#AFD8F5", borderWidth: 0.1 }}>
          <Image
            style={{ height: 200, aspectRatio: 1 / 1, resizeMode: "cover" }}
            source={{ uri: item.image }}
          />
          <View style={{ backgroundColor: "#006491", padding: 10,margin:5,marginTop:-5 }}>
            <Text style={{ fontSize: 15, fontWeight: "bold", color: "white" }}>
              {item.name.substr(0, 14)}
            </Text>

            <Text style={{ color: "pink", marginTop: 2 }}>
